    ...
    with open(csvFilePath, encoding='utf-8') as csvf:
        ...
        for rows in csvReader:
            try:
                ...
                logging.error(f"Key {e} not found in {csvFilePath}")
                continue
...
def main():
    ...
    for filename in os.listdir(config_folder):
        if filename.endswith('.csv'):
            csvFilePath = os.path.join(config_folder, filename)
            jsonFilePath = os.path.join(config_folder, filename.replace('.csv', '.json'))
            logging.info(f'Converting {csvFilePath} to {jsonFilePath}')
